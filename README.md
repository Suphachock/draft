...
class Model extends \Kotchasan\Model
{
    ...
    public static function send($order)
    {
        ...
        $query = static::createQuery()
            ->select('U.id', 'U.username', 'U.name', 'U.line_uid', 'RD.value', 'C.topic c_topic')
            ->from('user U')
            ->join('user_meta D', 'LEFT', array(array('D.member_id', 'U.id'), array('D.name', 'department')))
            ->join('reservation R', 'INNER', array('R.member_id', 'U.id'))
            ->join('reservation_data RD', 'INNER', array('RD.reservation_id', 'R.id'))
            ->join('category C', 'INNER', array(array('RD.name', 'accessories'),array('C.category_id','RD.value')) )
            ->where(array('U.active', 1))
            ->andWhere(array('C.type', 'accessories'))
            ->cacheOn();
        ...
        foreach ($query->execute() as $item) {
            if ($item->id == $order['member_id']) {
                ...
                $rd_value = $item->value;
                $reservation_value = $item->c_topic;
            } else {
                // เจ้าหน้าที่
        ...
        $msg = array(
            ...
            '{LNG_Status} : ' . $status,
            '{RD.value} : ' . $rd_value,
            '{C.topic} : ' . $reservation_value // Show the value from reservation_data
        );
